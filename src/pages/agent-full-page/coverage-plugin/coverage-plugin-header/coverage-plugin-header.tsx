...
export const CoveragePluginHeader = coveragePluginHeader(({ className }: Props) => {
  ...
  const { testTypeToNames = {} } = useBuildVersion<TestsToRun>({ topic: '/build/tests-to-run' }) || {};
  const conditionSettings = useBuildVersion<ConditionSetting[]>({ topic: '/data/quality-gate-settings' }) || [];
  const {
    status = 'FAILED',
    results = { coverage: false, risks: false, tests: false },
  } = useBuildVersion<QualityGate>({ topic: '/data/quality-gate' }) || {};
  const { coverage = 0, risks: risksCount = 0, tests: testToRunCount = 0 } = useBuildVersion<Metrics>({ topic: '/data/stats' }) || {};
